...
var _ = Describe("Errors test", func() {
	...
	Context("Marshalling", func() {
		...
		It("will be marshalled correctly with child errors", func() {
			...
			result := marshalHTTPError(httpErr)
			expected := `{"errors":[{"id":"001","links":{"about":"http://bla/blub"},"status":"500","code":"001","title":"Title must not be empty","detail":"Never occures in real life","source":{"pointer":"#titleField"},"meta":{"creator":"gitlab.dev.charm.internal/charm/api2go"}}]}`
			Expect(result).To(Equal(expected))
		})
